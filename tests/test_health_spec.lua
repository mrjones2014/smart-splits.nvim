...
describe('smart-splits.health', function()
  ...
  it('reports a version mismatch as an error', function()
    require('smart-splits').setup({ mux = { backend = helpers.mock_backend({ protocol_version = '99.0.0' }) } })
    Health.check()
    assert.is_true(reported_matching('error', 'protocol version 99.0.0'))
  end)
  ...
  it('reports the supported protocol versions', function()
    require('smart-splits').setup({})
    Health.check()
    assert.is_true(reported_matching('info', 'supported protocol version range'))
  end)

  it('reports an invalid backend as an error', function()
    require('smart-splits').setup({ mux = { backend = helpers.mock_backend({ move = false }) } })
    Health.check()
    assert.is_true(reported_matching('error', 'missing required field'))
  end)
end)
