...
describe('move_cursor', function()
  ...
  describe('backend delegation', function()
    ...
    -- the backend cannot see `at_edge`, and multiplexers like tmux wrap around
    -- their own edges by default, so core has to say whether that was asked for
    it('tells the backend that wrapping was asked for', function()
      local backend, calls = helpers.mock_backend()
      ss.setup({ mux = { backend = backend }, move = { at_edge = 'wrap' } })
      local wins = helpers.create_vsplits(2)
      helpers.focus(wins[2])
      ss.move_cursor_right()
      assert.same({ 'move', 'right', { wrap = true } }, helpers.find_call(calls, 'move'))
    end)

    it('tells the backend that wrapping was not asked for', function()
      local backend, calls = helpers.mock_backend()
      ss.setup({ mux = { backend = backend }, move = { at_edge = 'stop' } })
      local wins = helpers.create_vsplits(2)
      helpers.focus(wins[2])
      ss.move_cursor_right()
      assert.same({ 'move', 'right', { wrap = false } }, helpers.find_call(calls, 'move'))
    end)

    it('takes wrap from a per call at_edge override', function()
      local backend, calls = helpers.mock_backend()
      ss.setup({ mux = { backend = backend }, move = { at_edge = 'wrap' } })
      local wins = helpers.create_vsplits(2)
      helpers.focus(wins[2])
    ...
    it('reports wrap as false for at_edge = split', function()
      local backend, calls = helpers.mock_backend()
    ...
    it('asks the backend to split before splitting in nvim', function()
      local backend = helpers.mock_backend({
        split = function()
          return true
        end,
    ...
    it('splits in nvim when the backend cannot split', function()
      ss.setup({ mux = { backend = helpers.mock_backend({ split = false }) }, move = { at_edge = 'split' } })
      local wins = helpers.create_vsplits(2)
      helpers.focus(wins[2])
      local before = #vim.api.nvim_tabpage_list_wins(0)
      ss.move_cursor_right()
