...
describe('smart-splits.log', function()
  ...
  describe('levels', function()
    ...
    it('does not echo message below the configured level', function()
      local path = vim.fs.joinpath(tmpdir, 'below-level.log')
      Config.setup({ log = { level = 'warn', file = path } })
      local s = spy.on(vim.api, 'nvim_echo')
      Log.debug('a debug message')
      assert.spy(s).called(0)
      s:revert()
    end)

    it('still writes to file regardless of level', function()
      local path = vim.fs.joinpath(tmpdir, 'below-level.log')
      Config.setup({ log = { level = 'warn', file = path } })
      Log.debug('another debug message 2')
      vim.wait(100)
