...
`smart-splits.nvim` can cross from a Neovim edge split into a neighboring Herdr pane.
...
If you run Herdr inside `tmux`, and you want direct `ctrl+alt+h/j/k/l` pane navigation in non-Neovim Herdr panes,
you can make it work by configuring both Herdr and tmux.

Set the Herdr bindings in `~/.config/herdr/config.toml`:
...
If Herdr is running inside `tmux`, add the following to `~/.tmux.conf`/`~/.config/tmux/tmux.conf`:

```tmux
set -s extended-keys always
set -s extended-keys-format xterm
# Replace `xterm-kitty` with your tmux client terminal name if needed.
# Check it with: tmux display-message -p '#{client_termname}'
set -as terminal-features ",xterm-kitty:extkeys"

# Forward Ctrl+Alt+h/j/k/l to pane apps as raw CSI-u sequences that Herdr can parse.
bind -n C-M-h send-keys -H 1b 5b 31 30 34 3b 37 75
bind -n C-M-j send-keys -H 1b 5b 31 30 36 3b 37 75
bind -n C-M-k send-keys -H 1b 5b 31 30 37 3b 37 75
bind -n C-M-l send-keys -H 1b 5b 31 30 38 3b 37 75
```
...
- in Neovim panes, `<C-h/j/k/l>` moves between Neovim splits first, then crosses into Herdr panes at the edge
- in non-Neovim Herdr panes, `ctrl+alt+h/j/k/l` moves between Herdr panes directly
- `prefix+h/j/k/l` continues to work everywhere in Herdr as a fallback

> [!NOTE]
> This is not equivalent to the tmux integration. tmux can conditionally decide whether to send `<C-h/j/k/l>`
> into Neovim or use them for pane navigation. Herdr's documented keybinding config does not currently expose an
> equivalent pane-aware conditional.
>
> In particular, binding `ctrl+h/j/k/l` as Herdr direct keys or custom commands will cause Herdr to intercept
> those keys before Neovim sees them, so they are not drop-in replacements for the tmux setup.
>
> The `ctrl+alt+h/j/k/l` solution above is specifically for the `tmux -> herdr` case. The important piece is the
> raw `send-keys -H ...` forwarding in tmux; without it, tmux may recognize `C-M-h/j/k/l` itself but fail to pass
> a Herdr-compatible sequence into the pane.
>
> The reason tmux can do this more cleanly is that `smart-splits.nvim` writes pane-local state into tmux (the
> `@pane-is-vim` flag), and tmux key bindings can branch on that state before deciding whether to forward a key
> into the pane or to move between tmux panes. Herdr currently exposes direct bindings and custom commands, but not
> a documented equivalent of that pane-local conditional dispatch in its keybinding config. Once a key is bound in
> Herdr, Herdr handles it first.
