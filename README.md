...
Herdr support works from the Neovim side with `smart-splits.nvim`, but if you want the same navigation keys to work
in non-Neovim Herdr panes as well, you should add direct Herdr bindings.

Herdr's default pane movement keys are `prefix+h/j/k/l`. To get tmux-style "same keys everywhere" behavior, add direct
bindings in `~/.config/herdr/config.toml` while keeping the prefix bindings:
...
Then map the same keys in Neovim:
...
- `prefix+h/j/k/l` continues to work everywhere in Herdr

> [!NOTE]
> Herdr's own documentation recommends `ctrl+alt` as the safest direct-chord family across terminals and operating systems.
> Using plain `ctrl+h/j/k/l` as Herdr-wide direct bindings is usually a bad idea because those chords are commonly consumed by
> terminal applications, shells, and editors before Herdr sees them.
