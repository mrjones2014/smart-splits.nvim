...
vim.tbl_map(function(direction)
  M[string.format('resize_%s', direction)] = function(amount)
    ...
    if not ok then
      log.error(string.format('failed to resize: %s', error))
    end
  ...
  M[string.format('move_cursor_%s', direction)] = function(opts)
    ...
    if not ok then
      log.error(string.format('failed to move cursor: %s', error))
    end
  end
  M[string.format('swap_buf_%s', direction)] = function(opts)
    ...
    if not ok then
      log.error(string.format('failed to swap buffers: %s', error))
    end
  end
end, {
  Direction.left,
