...
local function call(op, ...)
  ...
  if elapsed > require('smart-splits.config').diagnostic.slow_threshold then
    Log.diagnostic('backend `%s` %s() took %.1fms, this runs on every keypress at an edge', backend.name, op, elapsed)
  end
